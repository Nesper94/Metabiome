...
##--------------------------------------Binning------------------------------------------------##:
##When coverage file is not provided:
if [[ ! -d "$cov_dir" ]];then
        for file in "$input_dir"/*;do
            if [[ "$file" == *.@(fna.gz|fasta.gz|fa.gz) ]];then
                    ...
                    metabat2 -i "$contig" -o "$out_dir"/"$core_name"/"$core_name" \
                        -t "$threads" $metabat2_opts
            fi
        done
#When coverage file is provided:
elif [[ -d "$cov_dir" ]];then
        for file in "$input_dir"/*;do
            if [[ "$file" == *.@(fna.gz|fasta.gz|fa.gz) ]];then
                    contig="$file"
                    core_name=$(get_core_name "$contig")
                    create_dir "$out_dir" "$core_name"
                    metabat2 -i "$contig" --abdFile "$cov_dir"/*"$core_name"* \
                        -o "$out_dir"/"$core_name"/"$core_name" -t "$threads" $metabat2_opts
            fi
        done
fi
